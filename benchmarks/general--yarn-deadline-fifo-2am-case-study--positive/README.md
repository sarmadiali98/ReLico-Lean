# `general--yarn-deadline-fifo-2am-case-study--positive`

A **general**-family positive benchmark for the YARN deadline FIFO model with two application
masters. A `ResourceManager` maintains a fixed three-job FIFO, dispatches to each free application
master in order, ages queued deadlines once per logical time unit, and records queue or execution
deadline misses.

## Provenance

- **Original source:** `examples.zip:ReLico-main/yarn-prop/yarn-deadline-fifo-2AMs.rebeca`.
...
The three-entry FIFO is scalarized, statically bounded loops are expanded, increment syntax is made
