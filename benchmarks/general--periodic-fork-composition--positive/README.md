...
9 stages, terminal `runtime`: `source`, `rmc`, `rmc-properties`, `parser-json`, `decoded-dtr-ast`, `translated-lf-ast`, `lf-source`, `lfc`, `runtime`. Source-level assertions check observed acknowledgement bounds and branch values and use an expected-`FALSE` counterexample to witness payload observations on the two state-changing branches.
