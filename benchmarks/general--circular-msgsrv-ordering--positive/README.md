# `general--circular-msgsrv-ordering--positive`

A **general**-family positive benchmark from the `examples2` corpus (`Deterministic_Circular_Ordering_messageserver.rebeca`), integrated without adaptation.

## Provenance

### 1. Original purpose

A deterministic cycle of message servers: sendMsg1 causes sendMsg2 causes sendMsg3 causes sendMsg4 causes sendMsg1, forever, in that order and no other.
