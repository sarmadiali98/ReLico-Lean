# `global-multi-actor-payload--initialization--positive`

A **global-multi-actor-payload**-family positive benchmark for initialization of several actors.

## Provenance

### 1. Original purpose

Formerly `global-multi-actor-payload--initialization--negative`, a negative benchmark. Several actors start together and exchange initial messages, exercising the startup alignment from source to target.

### 2. Why it was negative, and why that is obsolete

It was refused because reordered initialization was held to have no valid source-to-target startup alignment. That is a limitation of one family bridge, not a violation of the DTR subset: measured against the current verified fragment, this exact source passes the general parser bridge, the Lean decoder, the verified translation, LF compilation -- and the model checker reports `satisfied` for `Deadlock-Freedom and No Deadline Missed`. Per the stage K rule, a negative benchmark must represent a genuine violation of the current supported semantics, so the row is re-polarized rather than kept as a historical limitation.
...
Initialization of several actors, now as a positive witness end to end. The Lean obligation coverage the row already owned is preserved unchanged: 52 obligations across `GlobalMultiStorePayloadInitialization`.
