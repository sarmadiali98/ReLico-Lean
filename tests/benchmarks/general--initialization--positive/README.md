...
- parser JSON: produced by `frontend/java-bridge/run-general-from-zip.sh` through the `parser-json --family general` arm. Measured against the committed `frontend/fixtures/general/minimal-class.parser.json`, the two documents have identical shape and differ only in `line` fields, each shifted by the 5 header comment lines this benchmark source adds;
- decoded DTR AST, translated LF AST and LF source: the three `lean-export --family general` modes;
- formal witness: 18 obligations across `GeneralInitialization`;
