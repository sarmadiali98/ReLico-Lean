# `general--expression-evaluation--positive`

This positive benchmark belongs to the **general** family and its source is the corpus candidate `expressions`, selected in the stage K
...
- parser JSON: produced by `frontend/java-bridge/run-general-from-zip.sh` through the `parser-json --family general` arm. Measured against the committed `frontend/fixtures/general/expressions.parser.json`, the two documents have identical shape and differ only in `line` fields, each shifted by the 5 header comment lines this benchmark source adds;
- decoded DTR AST, translated LF AST and LF source: the three `lean-export --family general` modes;
- formal witness: 21 obligations across `GeneralEvaluation`;
