...
if [ "$#" -ne 3 ]; then
  echo "usage: run-multistore-from-zip.sh <artifact.zip> <input.rebeca> <output.json>" >&2
  exit 2
fi
...
EXPORTER_SOURCE="$(cd "$(dirname "$0")" && pwd)/RebecaMultiStorePayloadJsonExporter.java"
...
WORK_DIRECTORY="$(
  mktemp -d /tmp/relico-multistore-parser-bridge.XXXXXX
)"
...
cp \
  "$EXPORTER_SOURCE" \
  "$EXPORTER_DIRECTORY/RebecaMultiStorePayloadJsonExporter.java"
...
(
  ...
  "$MAVEN_BIN" \
    ...
    -Dexec.mainClass=org.rebecalang.compiler.frontendbridge.RebecaMultiStorePayloadJsonExporter \
...
echo "Multi-server parser bridge output: $OUTPUT_FILE"
